...
export default function CourseVideos() {
  ...
  const refreshSelectedCourse = async (slug) => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:5001/api/courses/slug/${slug}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleAddVideo = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        `http://localhost:5001/api/courses/${selectedCourse._id}/videos`,
  ...
  const handleDeleteVideo = async (videoId) => {
    try {
      ...
      const res = await axios.delete(
        `http://localhost:5001/api/courses/${selectedCourse._id}/videos/${videoId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      const res = await axios.put(
        `http://localhost:5001/api/courses/${selectedCourse._id}`,
