...
// import { AuthContext } from '../context/AuthContext'; // 👈 Same
...
//   const { user, setUser, loading: authLoading } = useContext(AuthContext); // 👈 NEW: authLoading ऐड किया
...
//     if (!user) return; // 👈 NEW: User न हो तो skip (prevent API call without auth)
//     setLoading(true);
//     setError(null);
//     try {
...
//         const referralCode = user.referralCode || "affiliate123"; // 👈 Fallback
...
//     }
//   }, [user, authLoading, navigate]); // 👈 UPDATED: Dependencies ऐड (authLoading और navigate)

//   // 👈 UPDATED: Global loading - authLoading || local loading
...
//             <img
//               src={user.profilePicture || "https://res.cloudinary.com/dxwtzb6pe/image/upload/v1757262791/oqwu4pod1xfyehprywc4.webp"} // 👈 UPDATED: Direct user.profilePicture (optional chaining remove, since !user check above)
//               alt={user.firstName || "User"} // 👈 Same
//               className="w-20 h-20 rounded-full object-cover border-2 border-gray-200"
//               onError={(e) => { // 👈 NEW: Image load error handle
...
//             {/* ... सभी CommissionCard same रहेंगे */}
//             <CommissionCard
...
//               amount={commissionStats.allTimeEarnings || 0} // 👈 Note: ये commissionStats.allTimeEarnings use कर रहा, fix if needed
//               color="green"
//               icon={
//                 <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
//               <p className="text-gray-800 font-medium">{user.email || 'N/A'}</p> {/* 👈 Fallback */}
...
//               <p className="text-gray-800 font-medium">
//                 {user.createdAt ? new Date(user.createdAt).toLocaleDateString() : 'N/A'} {/* 👈 Fallback */}
//               </p>
//               {commissionStats && (
//                 <div>
...
//                       commissionStats.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
//                     }`}
//                   >
...
//             <Input
//               value={referralLink || `Generating... (Code: ${user.referralCode || 'N/A'})`} // 👈 UPDATED: Fallback text
//               readOnly
