...
function UserHome() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        // ✅ Correct port 5001
        const userResponse = await fetch('http://localhost:5001/api/courses/enrolled', {
          headers: {
        ...
        // ✅ Correct port 5001
        const coursesResponse = await fetch('http://localhost:5001/api/courses/all');
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg">
            ...
            <p className="text-sm text-gray-600">
              <span className="text-orange-500">
                {`http://localhost:5001/all-courses?affiliateId=${user.affiliateId}`}
              </span>
            </p>
          </div>
        </section>
